...
#gas_two przez referencje

gas_two[, Values_Normalized := ((MeasuredValue - mean(MeasuredValue))/sd(MeasuredValue)), by = Pollutant]

#gas_two bez referencji

gas_two[, .(Values_Normalized = (MeasuredValue - mean(MeasuredValue))/sd(MeasuredValue)), by = Pollutant]



#dla latwiejszego zapisu przy gas_two_wide z lapply uzyje dodatkowej funkcji Normalized, która od razu normalizuje dana zmienna

Normalized = function(x){
  ((x - mean(x, na.rm = TRUE))/sd(x, na.rm = TRUE))
}

#gas_two_wide lapply() bez referencji

gas_two_wide[, .(c("Values_Normalized_Carbon", "Values_Normalized_Ozon"), lapply(.SD, Normalized)), .SDcols = c("Carbon monoxide", "Ozone")]

#gas_two_wide lapply() z referencja

gas_two_wide[, `:=` (c("Values_Normalized_Carbon", "Values_Normalized_Ozon") = lapply(.SD, Normalized)), .SDcols = c("Carbon monoxide", "Ozone")]

#gas_two_wide bez referencji

gas_two_wide[, .(Values_Normalized_Carbon = Normalized(`Carbon monoxide`), Values_Normalized_Ozon = Normalized(Ozone))]

#gas_two_wide z referencja
...
#konwertowanie do wersji szerokiej
gas_dt_wide <- dcast(gas_dt, State + County + City + Site + Date ~ year(Date),
                     value.var = "MeasuredValue", fill = NA_real_,
                     fun.aggregate = function(x) mean(x, na.rm = TRUE))

#powrot do waskiej
gas_dt_long <- melt(gas_dt_wide,
                    id.vars = setdiff(colnames(gas_dt_wide), c("2019", "2020")),
                    measure.vars = c("2019", "2020"),
                    variable.name = "Year", value.name = "MeasuredValue",
                    variable.factor = FALSE)

#konwertowanie do wersji szerokiej ze wzgledu na miasto
gas_dt_wide_cities <- dcast(gas_dt, State + County + Site + Date + Pollutant ~ City,
                            value.var = "MeasuredValue", fill = NA_real_,
                            fun.aggregate = function(x) mean(x, na.rm = TRUE))

#normalizacja dla kazdego miasta z osobna
uniqueCities = gas_dt_wide[,unique(City)]
gas_dt_wide_cities[, lapply(.SD, Normalized), .SDcols = uniqueCities]
...
gas_dt[, Mean := mean(MeasuredValue, na.rm = TRUE), by = c("State", "Pollutant", "Year")]