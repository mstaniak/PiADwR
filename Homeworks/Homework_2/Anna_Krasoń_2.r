...
#1
#z referencja
gas_two_ref = gas_two[,normMeasuredValue := (MeasuredValue - mean(MeasuredValue))/sd(MeasuredValue)]

# bez referencji
NormValue = gas_two[, (MeasuredValue - mean(MeasuredValue))/sd(MeasuredValue)]
gas_two_noref <- data.table(gas_two, NormValue)
...
gas_dt_long_year <- melt(gas_dt_wide_year,
                     id.vars = setdiff(colnames(gas_dt_wide_year), c("2020", "2019")),
                     measure.vars = c("2020", "2019"),
                     variable.name = "Date", value.name = "MeasuredValue",
                     variable.factor = FALSE)

#gas_dt_wide_city <- dcast(gas_dt, State + County + Date + Site + Pollutant ~ City,
#                      value.var = "MeasuredValue", fill = NA_real_,
#                      fun.aggregate = function(x) mean(x, na.rm = TRUE))

##3
gas_3 <- gas_dt[, AvgMeasured := mean(MeasuredValue, na.rm = TRUE),
       by = c("State", "County", "City", "Pollutant")]
